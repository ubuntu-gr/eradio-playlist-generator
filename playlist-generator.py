...
class PlaylistGenerator(object):
    ...
    def get_urlobject(self, src, sid):
        ...
        if not urlobject.getcode() == 200:
            ...
            exit()
            return False #skip
        #print("URL of urlobject: {0}\nHeaders: {1}".format(urlobject.url,urlobject.headers.dict))
        return urlobject
    ...
    def check_content_type(self, src, sid, mms=False):
        ...
        if r.headers.has_key("Content-Type"):
            ...
            # TODO - use python-requests
            import requests
            r = requests.get(src, verify=False, stream=True, timeout=10)
            print("python-requests url:{0}\nheaders: {1}".format(r.url, r.headers))
            readcontent = r.raw.read(200)
            print("partial content: {0}".format(readcontent))
            if readcontent.startswith('ICY 401 Service Unavailable') or readcontent.startswith('Service Unavailable'):
                print("Error: ICY 401 Service Unavailable: {0} // {1}".format(src, r.url))
                self.add_to_blacklist(sid)
                return False
            if readcontent.startswith('ICY 403 Service Forbidden'):
                print("Error: ICY 403 Service Forbidden: {0} // {1}".format(src, r.url))
                self.add_to_blacklist(sid)
                return False
            if readcontent.startswith('ICY 404 Resource Not Found'):
                print("Error: ICY 404 Resource Not Found: {0} // {1}".format(src, r.url))
                self.add_to_blacklist(sid)
                return False
            if readcontent.startswith('ICY 200 OK'):
                print("Matched ICEcast server {0} // {1}".format(src, r.url))
                self.add_to_radiodb(src, sid)
                return True
            else:
                try:
                    ...
                    if ctype and ctype.startswith("audio/"):
                        print("Matched content-type: {0} {1}".format(src, ctype))
                        self.add_to_radiodb(src, sid)
                        return True
                except KeyError:
...
def go(service):
    if service == "eradio":
        ...
        service_template = {
            ...
            "blacklist"       : ['818', '819', '347', '342', '343', '810', '811', '815', '718', '606', '619', '298', '296', '297', '595', '427', '295', '292', '591', '592', '594', '596', '194', '196', '190', '738', '524', '525', '526', '527', '528', '449', '442', '441', '440', '447', '445', '444', '108', '109', '102', '100', '101', '104', '907', '36', '35', '641', '645', '88', '648', '435', '433', '1002', '334', '337', '336', '332', '558', '98', '93', '92', '95', '94', '97', '153', '742', '745', '556', '550', '553', '234', '237', '230', '231', '232', '233', '144', '145', '143', '613', '611', '617', '946', '944', '945', '943', '940', '689', '684', '683', '682', '133', '131', '136', '135', '495', '490', '491', '492', '21', '407', '406', '404', '403', '400', '409', '379', '378', '376', '709', '705', '700', '701', '397', '395', '394', '83', '80', '398', '84', '85', '797', '794', '793', '798', '170', '585', '583', '581', '245', '246', '241', '148', '615', '248', '518', '511', '1006', '512', '517', '623', '620', '627', '624', '178', '174', '198', '183', '180', '182', '654', '181', '653', '184', '652', '188', '187', '947', '659', '568', '116', '657', '326', '325', '328', '774', '204', '773', '206', '208', '779', '76', '75', '74', '655', '358', '669', '667', '662', '660', '215', '692', '693', '690', '697', '694', '699', '543', '540', '546', '548', '990', '120', '128', '214', '415', '416', '410', '413', '137', '319', '313', '310', '317', '316', '314', '496', '367', '952', '380', '381', '382', '383', '384', '385', '386', '387', '786', '781', '782', '789', '578', '572', '577', '576', '575', '574', '258', '69', '255', '603', '730', '735', '507', '502', '633', '635', '636', '637', '465', '461', '462', '164', '165', '167', '160', '963', '1103', '891', '896', '356', '355', '802', '800', '804', '217', '768', '769', '762', '42', '760', '957', '321', '280', '677', '671', '672', '673', '261', '260', '264', '1031', '436', '58', '55', '57', '56', '51', '52', '537', '536', '63', '533', '532', '539', '201', '988', '50', '115', '112', '118', '207', '429', '428', '918', '916', '302', '303', '305', '306', '954', '756', '758', '564', '567', '506', '222', '220', '1026', '114', '1028', '503', '720', '601', '602', '609', '469', '468', '959', '48', '49', '43'],
        }
